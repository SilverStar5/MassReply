...
OAuth2.prototype.updateLocalStorage = function() {
    ...
    var data = {};
    var variables = ['accessToken', 'accessTokenDate', 'apiScope', 'clientId', 'clientSecret', 'expiresIn', 'refreshToken'];
...
/*OAuth2.prototype.clearStorage = function() {
    for (var i = 0; i < variables.length; i++) {
        key = this.adapterName + '_' + variables[i];
        if (localStorage.hasOwnProperty(key)) {
            data[variables[i]] = localStorage[key];
    ...
    xhr.onreadystatechange = function(event) {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                callback(obj.access_token, obj.expires_in, refreshToken);
            }
        }
    };
    var data = this.get();
 ...
 */
OAuth2.prototype.finishAuth = function(callback) {
